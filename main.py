...
def snap_cam():
    ...
    if grabResult.GrabSucceeded():
        ...
        img = image.GetArray()
        np_img = cv2.resize(img, dsize=(448, 448), interpolation= cv2.INTER_CUBIC)
        Gui.im = Image.fromarray(np.uint8(np_img))
        name = 'img-01.bmp'
        Gui.im.save(name)
        Gui.imPIL = ImageTk.PhotoImage(file ='img-01.bmp')
        dis_label.configure(image = Gui.imPIL)
...
if __name__ == "__main__":
    ...
    img_display = LabelFrame(Gui,width = screen_size[0]*0.8, height = screen_size[1]*0.62,font=('TkDefaultFont', 40),text = "Display Image")
